package com.wuzhiwei.bigdata.project.etl

import java.util
import java.util.zip.CRC32

import com.wuzhiwei.bigdata.project.common.EventLogConstants
import com.wuzhiwei.bigdata.project.common.EventLogConstants.EventEnum
import com.wuzhiwei.bigdata.project.util.{LogParser, TimeUtil}
...
/**
  ...
  */
object EtlToHBaseHFileSnapShotSpark {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 1) {
      println("Usage: EtlToHBaseHFileSnapShotSpark process_date")
      System.exit(1)
    }
    ...
    val sparkConf = new SparkConf()
      .setMaster("local[3]").setAppName("EtlToHBaseHFileSnapShotSpark Application")
      // 设置使用Kryo序列化
    ...
    // TODO: 批量加载HFiles数据文件到HBase表中以后，给表创建快照
    createHBaseTableSnapshot(tableName, conf)
  ...
  /**
    * 当将HFile数据文件加载到HBase表中以后，给当前表生成快照snapshot，以便后续分析
    * @param tableName
    *                  表的名称
    */
  def createHBaseTableSnapshot(tableName: String, conf: Configuration): Unit ={
    ...
    try{
      ...
      // 创建表的快照
      // public Unit snapshot(snapshotName: String, tableName: String)
